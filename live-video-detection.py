...
#
# # Configuration
# TFLITE_MODEL_PATH = "yolov8n_saved_model/yolov8n_int8.tflite"
# INPUT_SIZE = 640
...
#     """Resize, normalize, and add batch dimension to frame."""
#     img = cv2.resize(frame, (INPUT_SIZE, INPUT_SIZE))
#     img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB) / 255.0
#     return np.expand_dims(img.astype(np.float32), axis=0)
#
# def postprocess(output_data, frame_shape):
#     """
#     Convert raw model output into bounding boxes, scores, and class IDs.
#     Assumes output_data shape: (1, 84, 8400).
#     """
#     data = np.squeeze(output_data)  # shape: (84, 8400)
#     boxes = data[:4, :].T           # (8400, 4): cx, cy, w, h normalized
#     scores = data[4, :]             # (8400,)
#     class_probs = data[5:, :]       # (num_classes, 8400)
#     classes = np.argmax(class_probs, axis=0)
#     valid = scores > CONFIDENCE_THRESHOLD
#
#     h, w = frame_shape[:2]
#     bbs, scs, cls = [], [], []
#     for box, score, c, ok in zip(boxes, scores, classes, valid):
#         if not ok:
#             continue
#         cx, cy, bw, bh = box
#         x1 = int((cx - bw/2) * w)
...
#     for (x1, y1, x2, y2), score, c in zip(boxes, scores, class_ids):
...
#
#     # Display the frame
#     cv2.imshow("Live TFLite Detection", frame)
